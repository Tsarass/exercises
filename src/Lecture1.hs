...
subString start end str
    ...
    | end <= start = ""
    | otherwise = drop start (take end str)
...
-}
strSum :: [Char] -> Int
strSum str = sum [read w | w <- (words str)]
...
lowerAndGreater n list = show n ++ " is greater than " ++ show lowers ++ " elements and lower than " ++ show highers ++ " elements"
    where
        lowers = sum [1 | x <- list, x < n]
        highers = sum [1 | x <- list, x > n]
